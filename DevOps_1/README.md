...
Показалось, что ничего не поменялось, но чистка кэша дала свои плоды:
...
```
5. Создадим отдельную директорию для сертификатов, скопируем их туда, создадим `dhparam` для обеспечения большей безопасности и поменяем права на файл, чтобы `nginx` не ругался:
```
...
6. Перепишем конфиги для `nginx`, удалив сниппеты и добавив пути к сертификату и ключу:
```
server {
	listen 80;
	listen [::]:80;
...
Проекты работают только когда пользователь вводит `insect-catch-game.com/static/`, это неудобно, поэтому с помощью alias'а переопределим путь к файлам:
```
location / {
...
4. Определён `alias` для удобства пользователей
